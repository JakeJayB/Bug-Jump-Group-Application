...
public class MainGame extends GraphicsPane {
	...
	/**
	 * As soon as one key is released from the keyboard, it is removed from the list
	 * of all keys held down by the user
	 */
	@Override
	...
	/**
	 * ONLY Checks player's left, right, top, and bottom collision. work
	 * hand-on-hand with objectPlayerCollision()
	 * 
	 * @return true if player is colliding with a wall. False otherwise
	 */
	private boolean checkCollision() {
		// was 6 before
		if (objectPlayerCollision(
				new GObject[] { program.getElementAt(player.getX() + .333 * playerWidth, player.getY() - 4),
		...
		if (objectPlayerCollision(new GObject[] { program.getElementAt(player.getX() + 2, player.getY() + 54), // was 54
																												// before
		...
		if (objectPlayerCollision(new GObject[] { program.getElementAt(player.getX() - 6, player.getY()),
			...
			return true;
		} else if (objectPlayerCollision(
				new GObject[] { program.getElementAt(player.getX() + playerWidth + 6, player.getY()),
	...
	/**
	 * 
	 * @param arr an array of GObjects used to check if they exist. These are our
	 *            player collision points
	 * @return false if all GObjects are null or player is colliding with a enemy,
	 *         or collectable.
	 *         True if one or more collision points aren't null and are colliding
	 *         wih a wall
	 * 
	 */
	private boolean objectPlayerCollision(GObject[] arr) {
		...
		for (GObject gImage : arr) {
			if (gImage == background) {
				nullCount++;
				continue;
			}

			if (collectablesMap.containsKey(gImage)) {
				// Checks Map for which collectable is associated to which gImage and switches
				// to perform
				// effects accordingly
	...
	private void updateBullet() {
		...
		for (Entry<GImage, Bullet> entry : bulletMap.entrySet()) {
			...
			if (checkBulletCollision(key, val)) {
	...
	private boolean checkBulletCollision(GImage key, Bullet val) {
		...
		if (val.isFriendly() && ((enemiesMap.containsKey(obj1)) || enemiesMap.containsKey(obj2)
				|| enemiesMap.containsKey(obj3) || enemiesMap.containsKey(obj4))) {
			Enemy enemy = null;
			GObject temp = null;
			...
			if (enemy.isDead()) {
				program.remove(temp);
				enemiesMap.remove(temp);
			}
			return true;
		}

		if (val.isMelee() == false && (terrainMap.containsKey(obj1) || terrainMap.containsKey(obj2)
				|| terrainMap.containsKey(obj3) || terrainMap.containsKey(obj4))) {
			return true;
		}
		return false;
	...
	private void doEnemyActions() {
		...
		for (Entry<GImage, Enemy> entry : enemiesMap.entrySet()) {
			...
			if (each.getAwareness()
				...
				if (bulletMap.containsKey(program.getElementAt(each.getX() - 2, each.getY() + 52))
						|| program.getElementAt(each.getX() - 2, each.getY() + 52) == background ||
						program.getElementAt(each.getX() - 2, each.getY() + 52) == null
						|| terrainMap.containsKey(program.getElementAt(each.getX() - 2, each.getY()))) {
					each.setIsRightOrientation(true);
					changeEnemyImage(each, eachImage);
				} else if (bulletMap.containsKey(program.getElementAt(each.getX() + 52, each.getY() + 52))
						|| program.getElementAt(each.getX() + 52, each.getY() + 52) == background
						|| program.getElementAt(each.getX() + 52, each.getY() + 52) == null
						|| terrainMap.containsKey(program.getElementAt(each.getX() + 52, each.getY()))) {
					each.setIsRightOrientation(false);
					changeEnemyImage(each, eachImage);
				}
	...
	private void changeEnemyImage(Enemy each, GImage image) {
		...
		if (enemyType != EnemyType.WORM && enemyType != EnemyType.BEETLE) {
			return;
		}
	...
	private void enemyAwareness() {
		...
		for (Entry<GImage, Enemy> entry : enemiesMap.entrySet()) {
			...
			if (all.getEnemyType() != EnemyType.BEETLE && all.getEnemyType() != EnemyType.FLOWER) {
				continue;
			}
	...
	public void setupPauseGameScreen() {
		...
		resumeButton = new GButton("Resume", dimension.getWidth() / 2 - 150, pause.getY() + 75, 300, 75,
				Color.decode("#879383"));
		program.add(resumeButton);

		mainMenuButton = new GButton("Main Menu", dimension.getWidth() / 2 - 150, resumeButton.getY() + 100, 300, 75,
				Color.decode("#879383"));
	...
	public void setupWinningScreen() {
		...
		victory.setLocation(dimension.getWidth() / 2 - victory.getWidth() / 2,
				victoryBorder.getY() + victory.getHeight());
		program.add(victory);

		nextLevelButton = new GButton("Next Level", dimension.getWidth() / 2 - 110,
				victory.getY() + victory.getHeight() / 2, 220, 70, Color.decode("#879383"));
		program.add(nextLevelButton);

		mainMenuButton = new GButton("Main Menu", dimension.getWidth() / 2 - 187.5,
				nextLevelButton.getY() + nextLevelButton.getHeight() + 10, 375, 90, Color.decode("#879383"));
		program.add(mainMenuButton);
	...
	public void setupGameOverScreen() {
		...
		restartButton = new GButton("Restart", dimension.getWidth() / 2 - 110, over.getY() + over.getHeight() / 2, 220,
				70, Color.decode("#879383"));

		program.add(restartButton);

		mainMenuButton = new GButton("Main Menu", dimension.getWidth() / 2 - 187.5,
				restartButton.getY() + restartButton.getHeight() + 10, 375, 90, Color.decode("#879383"));
		program.add(mainMenuButton);

	}

	/**
	 * Takes in the current save file and updates the save file with the stars
	 * gotten on the level.
	 * 
	 * @throws IOException
	 */
	private void updateStars() {
		if (level != 0) {
			...
			if (Integer.parseInt(lineArr[level - 1]) >= stars) {
				return;
			}
